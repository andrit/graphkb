...
| Processing | Chunk text, extract entities/relationships, generate metadata and embeddings | Phase 1: TypeScript-native (heuristic NER). Phase 2: Python services (spaCy, sentence-transformers) |
...
## 9. Ingestion Pipeline (Implementation)

> Added 2026-04-13. Closes open question: "Ingestion pipeline detail — how does content flow from upload to graph?"

The ingestion pipeline is the system's core data path. It accepts a file upload, transforms it into structured knowledge, and writes that knowledge to both Neo4j and Elasticsearch in a single Effect chain. In Phase 1 the entire pipeline runs synchronously within the API process (no BullMQ workers yet). The pipeline is implemented across five modules in `@rhizomatic/ingestion`, orchestrated by `pipeline.ts`.

### 9.1 Pipeline stages

The pipeline is a single composable `Effect` value with the signature:

```typescript
Effect.Effect<
  IngestionResult,
  ValidationError | StorageError | GraphError | SearchError,
  FileStorage | GraphClient | SearchClient
>
```

All four error types and all three service dependencies are tracked in the type. Nothing is thrown; nothing is implicitly available.

**Stage 1 — Validate and detect content type** (`validation.ts`)

Checks that the file is non-empty and under 100 MB. Maps the file extension to a `ContentType` value using a static lookup table. Supported extensions: `.md`, `.txt`, `.html`, `.htm`, `.csv`, `.pdf`, `.docx`, `.xlsx`, `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`. Unsupported extensions fail with a `ValidationError` that names the extension.

**Stage 2 — Store original file** (`@rhizomatic/storage`)

Writes the raw file bytes to local storage using content-addressable hashing. The file is stored at `data/files/<first-2-hex-chars>/<sha256-hash>` — the hash is the storage key, so identical files are automatically deduplicated. The `FileStorage` service is injected via Effect's dependency system; swapping to S3 means replacing the Layer, not the pipeline.

**Stage 3 — Extract text** (`extractors.ts`)

Dispatches to a content-type-specific adapter function. Each adapter takes a `Buffer` and `fileName` and returns an `ExtractedContent` — the anti-corruption boundary between messy external formats and clean domain types. `ExtractedContent` contains: `text` (full plain text), `title`, `metadata`, `sections` (heading-delimited blocks with `heading`, `content`, `level`, `position`), and optional `tabularData`.

Current adapters:

| Content type | Adapter | Strategy |
|---|---|---|
| `markdown` | `extractMarkdown` | UTF-8 decode, heading detection via `#` and underline patterns, section parsing |
| `html` | `extractHtml` | Strip `<script>`/`<style>`, convert block elements to newlines, strip remaining tags, decode entities, extract `<title>`, parse `<h1>`–`<h6>` into sections |
| `csv` | `extractCsv` | RFC-compliant line parser (handles quoted fields, escaped quotes), produces `tabularData` and a key-value text representation for NLP (`"Header: value, Header: value"` per row) |
| `pdf`, `docx`, `xlsx`, `image` | `extractPlainText` (fallback) | UTF-8 decode only. These are Phase 1 stubs — binary formats will be handled by Apache Tika |

The adapter registry is a simple `Record<ContentType, Extractor>` map. Adding a new content type means writing one function and adding one entry.

**Stage 4 — Chunk text** (`chunker.ts`)

Splits extracted text into `ProcessedChunk` values suitable for entity extraction and search indexing. Pure function, no dependencies.

Chunking strategy:
- If sections are available (from heading detection in Stage 3), each section is chunked independently — section boundaries are never crossed.
- Within each section (or for unsectioned text), text is split at paragraph boundaries (double newlines). Paragraphs are never split mid-paragraph.
- Paragraphs are greedily grouped until adding the next paragraph would exceed the maximum chunk size.
- Chunks below the minimum size threshold are discarded (noise filtering).

Configuration (defaults):

| Parameter | Default | Purpose |
|---|---|---|
| `maxChunkSize` | 1200 chars | Target ceiling per chunk |
| `minChunkSize` | 50 chars | Floor — shorter chunks are dropped |
| `overlapSize` | 100 chars | Reserved for future overlap between adjacent chunks (not yet implemented) |

**Stage 5 — Extract entities and relationships** (`entities.ts`)

TypeScript-native NER using pattern matching and heuristics. No Python or spaCy dependency in Phase 1. The interface (`ExtractedEntity`, `ExtractedTriple`) is stable — replacing the implementation with spaCy workers in Phase 2 requires no changes to the pipeline orchestrator.

Three extraction strategies run per chunk:

1. **Capitalized noun phrases.** Regex matches sequences of capitalized words (e.g., "John Hughes", "Higher Order Functions"). Multi-word phrases get confidence 0.75; single capitalized words get 0.50 and are filtered more aggressively (sentence-start capitalization is excluded). A comprehensive stop-word list (200+ entries including months, days, and academic section headers) prevents false positives.

2. **Known technology terms.** A curated set of ~80 terms (languages, frameworks, databases, protocols, tools) is matched case-insensitively against the text. Matches get confidence 0.90. The set is extensible.

3. **Quoted terms.** Phrases in double or single quotes that start with a capital letter are extracted as entities with confidence 0.65.

Entity kind inference uses a cascade: check the technology term set → check organizational suffixes ("Inc", "Corp", "University") → check person prefixes ("Dr", "Prof") → check contextual keywords within ±50 characters ("wrote"/"said" → person, "company"/"startup" → organization, "language"/"framework" → technology) → two-word capitalized phrase → default to "concept".

**Relationship discovery** uses co-occurrence: all pairs of entities found in the same chunk produce a `RELATED_TO` triple with confidence = `entity_a.confidence × entity_b.confidence × 0.8` (multiplicative decay prevents weak entities from generating strong relationships). Duplicate pairs across chunks are deduplicated by canonical name.

Entity deduplication across chunks uses the lowercased canonical name as the key, keeping the higher-confidence entry.

**Stage 6 — Write to Neo4j** (pipeline.ts → `GraphClient`)

Creates the graph structure in a sequence of Cypher writes:

1. `:Document` node with `id`, `title`, `contentType`, `fileHash`, `ingestedAt`, `summary`, `metadata`.
2. `:Chunk` nodes with `HAS_CHUNK` edges from the document (with `position`).
3. `NEXT_CHUNK` edges between consecutive chunks for reading order.
4. `:Source` node (MERGE — reuses the "File Uploads" source across all uploaded files) with `FROM_SOURCE` edge.
5. `:Entity` nodes via MERGE — creates new entities or updates existing ones (entity resolution by name). Aliases are accumulated.
6. `MENTIONS` edges from each chunk to its discovered entities (with `confidence`).
7. `RELATED_TO` edges between co-occurring entities (with `weight`, `kind: "co-occurrence"`, `source: "auto"`, `createdAt`).

All writes go through the `GraphClient` Effect service. The pipeline does not construct raw Cypher — it uses query builders from `@rhizomatic/graph` (`queries.createDocument`, `queries.mergeEntity`, `queries.createMention`, `queries.relateEntities`).

**Stage 7 — Index in Elasticsearch** (pipeline.ts → `SearchClient`)

Indexes three document types in parallel across three ES indexes:

| Index | ES index name | Fields indexed |
|---|---|---|
| Documents | `rhizomatic-documents` | `id`, `title`, `contentType`, `summary`, `ingestedAt` |
| Chunks | `rhizomatic-chunks` | `id`, `content`, `heading`, `documentId`, `position` (embedding field reserved for Phase 2) |
| Entities | `rhizomatic-entities` | `id` (derived from name), `name`, `aliases`, `kind`, `mentionCount` |

Entity ES IDs are deterministic (`ent_<lowercased_underscored_name>`), so re-ingesting a document that mentions the same entity updates the existing ES document rather than creating a duplicate.

### 9.2 Pipeline return value

The pipeline returns an `IngestionResult` summary:
...
This is surfaced directly to the frontend after upload and also returned via the GraphQL `IngestResult` type.

### 9.3 HTTP integration

The Fastify server exposes `POST /upload` as a multipart endpoint (via `@fastify/multipart`, 100 MB limit). The route handler:

1. Reads the file buffer and filename from the multipart data.
2. Calls `runIngestionPipeline(buffer, fileName)`.
3. Provides the three required Effect services (`GraphClient`, `SearchClient`, `FileStorage`) via `Effect.provideService`.
4. Runs the Effect to a Promise and returns the `IngestionResult` as JSON.

Error handling: any pipeline failure (validation, storage, graph, or search) is caught and returned as an HTTP 500 with the error message. In Phase 2, pipeline execution will move to a BullMQ job with status tracking, and the upload endpoint will return a job ID for polling.

### 9.4 Frontend integration

The upload page (`packages/web/src/app/ingest/page.tsx`) provides drag-and-drop file upload with state machine UX: `idle → uploading → done | error`. Upload history is maintained in component state for the session. The `packages/web/src/lib/api.ts` module provides a typed `uploadFile(file: File)` function that POSTs to `/upload` and returns a typed `IngestionResult`.

### 9.5 Current limitations and Phase 2 migration path

| Limitation | Phase 2 solution |
|---|---|
| Binary format extraction (PDF, DOCX, XLSX, images) uses UTF-8 fallback | Apache Tika integration in `services/processor` |
| Entity extraction is heuristic pattern-matching | spaCy NLP workers via BullMQ |
| Pipeline runs synchronously in the API request | BullMQ job queue with status polling |
| No embeddings generated | sentence-transformers in `services/embedder`, stored in ES `dense_vector` field (already mapped) and Qdrant |
| No chunk overlap for context continuity | `overlapSize` config is defined but not yet applied |
| Entity resolution is name-only | Alias-aware resolution, Wikidata enrichment |

The critical design choice is that **the interface types are stable**. `ExtractedContent`, `ProcessedChunk`, `ExtractedEntity`, and `ExtractedTriple` are defined in `@rhizomatic/common` and shared across all implementations. Replacing the TS-native NER with spaCy means writing a new implementation behind the same `extractEntities` function signature — the pipeline orchestrator, graph writer, and ES indexer are unchanged.

---

## 10. Graph Data Model (Neo4j Ontology)
...
### 10.5 Cypher schema initialization
...
### 10.6 Concrete example: ingesting a PDF
...
### 10.8 Category theory as a design lens
...
### 10.9 Typed relationship composition (core feature)
...
### 10.10 Committed tools and technologies roadmap
...
- [x] Ingestion pipeline detail — how does content flow from upload to graph? *(Answered in Section 9)*
- [x] Search strategy — how do full-text, graph traversal, and vector search combine? *(Phase 1: full-text search across three ES indexes merged by score; graph traversal via GraphQL resolvers; vector search deferred to Phase 2)*
- [x] Wiki page structure — how do graph nodes map to browsable pages? *(Implemented: `/wiki/doc/[id]` for documents, `/wiki/entity/[name]` for entities)*
...
- [ ] Tika vs. custom adapters — evaluate extraction quality tradeoffs during scaffolding *(Phase 1 uses TS-native adapters for text formats + UTF-8 fallback stubs for binary formats; Tika integration planned for Phase 2)*
